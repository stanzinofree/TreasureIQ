"""Golden tests for the resolved-service cache (Ramo 3, Slice 2).
...
# 1 — salva then fresh carica returns the same reference (mixed options round-trip)
def test_salva_poi_carica_fresco():
    ref = _reference()
    service_cache.salva("058003", ServiceKey.CARTA_IDENTITA, ref)
...
def test_carica_fonte_o_chiave_assente():
    ...
    service_cache.salva("058003", ServiceKey.CARTA_IDENTITA, _reference())
...
# 4 — entry with an older schema_version → None (re-read)
...
# 4b (added) — entry with a FUTURE schema_version → None (not safely readable)
...
# 5b (added) — file whose internal source_id disagrees with the filename → miss
...
def test_no_clobber_altre_chiavi():
    ...
    service_cache.salva("058003", ServiceKey.CARTA_IDENTITA, ref_cie)
    service_cache.salva("058003", ServiceKey.TRIBUTI, ref_tributi)
    assert service_cache.carica("058003", ServiceKey.CARTA_IDENTITA, policy=_POLICY) == ref_cie
    assert service_cache.carica("058003", ServiceKey.TRIBUTI, policy=_POLICY) == ref_tributi


# 6b — re-salva of the same key overwrites only that key
def test_re_salva_stessa_chiave():
    service_cache.salva("058003", ServiceKey.CARTA_IDENTITA, _reference("v1"))
    service_cache.salva("058003", ServiceKey.CARTA_IDENTITA, _reference("v2"))
    letto = service_cache.carica("058003", ServiceKey.CARTA_IDENTITA, policy=_POLICY)
    assert letto is not None and letto.service_id == "v2"


# 7 — same service_key, two source_id → isolated entries
def test_isolamento_per_fonte():
    service_cache.salva("058003", ServiceKey.CARTA_IDENTITA, _reference("albano"))
    service_cache.salva("058091", ServiceKey.CARTA_IDENTITA, _reference("marino"))
    ...
    assert a is not None and a.service_id == "albano"
    assert b is not None and b.service_id == "marino"
...
def test_salva_mount_non_scrivibile(monkeypatch, tmp_path):
    ...
    service_cache.salva("058003", ServiceKey.CARTA_IDENTITA, _reference())  # must not raise
...
# 10 (added) — invalid source_id / path traversal → rejected (raise)
...
def test_source_id_non_valido_rifiutato(cattivo):
    ...
    with pytest.raises(ValueError):
        service_cache.salva(cattivo, ServiceKey.CARTA_IDENTITA, _reference())


# 11 (added) — naive and aware timestamps normalise the same (both fresh)
