...
Requires `BRAVE_SEARCH_API_KEY` in the environment (D-59: Brave is the only
engine, no SearXNG fallback). `--pausa` can drop to a second or two — the
quota is published and there is no bot detection to trip, unlike the old
meta-search fallback.

Why the pacing/retry machinery exists rather than a plain loop: the old
SearXNG fallback rate-limited hard under load. Forty queries fired back to
back once earned `suspended_time=180` from both Google CSE and Brave, after
which every search returned zero results — indistinguishable, from the
caller's side, from "this comune has published nothing", which is the one
confusion this project cannot afford. So requests stay paced, empty answers
are retried after a longer wait, and anything still empty at the end is
reported as unresolved rather than cached as an absence.
...
def main(argv: list[str] | None = None) -> int:
    ...
    for i, (query, topic, comune) in enumerate(lavori, 1):
        ...
        for tentativo in range(1, args.tentativi + 1):
            ...
            try:
                entry = run_and_cache(query, cache_dir=cache, timeout=30.0)
